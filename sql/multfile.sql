...
CREATE TABLE `multfile`.files (
    file_id CHAR(36) PRIMARY KEY,             -- 文件唯一标识（UUID）
    file_name VARCHAR(255) NOT NULL,          -- 文件名
    file_path VARCHAR(255) NOT NULL,         -- 文件路径
    file_size BIGINT NOT NULL,                -- 文件大小
    owner_id CHAR(36) NOT NULL,               -- 文件所有者（UUID）
    ...
    INDEX idx_files_file_name (file_name),    -- 文件名索引
    INDEX idx_files_owner_id (owner_id),      -- 所有者索引
    INDEX idx_files_created_at (created_at),  -- 创建时间索引
    INDEX idx_files_owner_created (owner_id, created_at) -- 复合索引
